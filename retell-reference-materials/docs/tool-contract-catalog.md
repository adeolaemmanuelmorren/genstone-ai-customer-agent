...
| **Find active employee** — Resolve a caller-supplied employee name to an eligible direct transfer number. | `POST /v1/retell/tools/employees/lookup` / `lookup_active_employee` | `call_id`; employee name | Salesforce User | `found`, `not_found`, `ambiguous`, `missing_number`, `error` | Return a transfer token/dynamic value, not the direct number for the agent to speak. Only active Users in a mapped GenStone profile with a direct phone qualify. |
| **Find order candidate** — Find the candidate used by the shared existing-order verification gate. | `POST /v1/retell/tools/orders/lookup` / `lookup_order`, `lookup_order_alternate`, or `lookup_next_order` | `call_id`; confirmed phone, alternate phone, order number, or previous rejected candidate | WooCommerce | `found`, `not_found`, `ambiguous`, `validation_failed`, `error` | Exclude quote-status drafts from phone and exact order-number lookup. Identify stored sample orders and explicitly signaled retail orders in the confirmation. Ask for the order number at most once; if unavailable, continue through the remaining recent non-quote candidates without asking again. The token is not proof of verification. Downstream order tools require item confirmation and `order_verified=true`. |
...
| Name / description | Proposed route / Retell name | Required input | System | Caller-safe outcomes | Boundary |
| --- | --- | --- | --- | --- | --- |
| **Send unmatched prospect follow-up** — Send a new prospect who requested information that the agent could not provide and was not found in Salesforce to Travis for internal follow-up. | `POST /v1/retell/tools/prospects/follow-up` / `send_prospect_follow_up` | `call_id`, idempotency key, `primary_route=new_project`, confirmed name, phone, and email, project summary, optional volunteered postal code | Customer.io | `sent`, `validation_failed`, `delivery_failed`, `error` | New projects only, after Salesforce contact lookup returns `not_found`. The caller's request is sufficient authorization; do not repeat other contact details or require a formal summary read-back. Internal email to Travis; never creates a Salesforce Lead or Contact and never emails the customer. |
| **Schedule centralized callback** — Send a new-project caller's confirmed preferred callback request internally. | `POST /v1/retell/tools/callbacks/schedule` / `schedule_callback` | `call_id`, idempotency key, `primary_route=new_project`, confirmed caller name, phone, and email, subject, summary, preferred date/time | Customer.io | `scheduled`, `invalid_day_or_time`, `delivery_failed`, `error` | New projects only. Next business day or later; Monday-Friday 8:30 AM-4:30 PM Mountain; internal email only. The backend rejects existing-order callback requests. |
...
| **Create existing-order follow-up** — Create one new private Zendesk answering-service ticket, then send the required internal notice. | `POST /v1/retell/tools/support/cases` / `create_support_case` | `call_id`, idempotency key, `primary_route=existing_order`, confirmed name, phone, and email, factual issue summary, caller type, and country when volunteered | Zendesk + Customer.io | `created`, `created_notice_failed`, `validation_failed`, `error` | The caller's request for customer-service help is sufficient authorization; do not read the issue back or ask permission to create internal work. Never search for or update an earlier ticket during the call. Attach the confirmed customer name and email as the Zendesk requester. Use a private initial comment; the application sends no customer email. Support group, no assignee, normal priority; native Type `Question`; Ticket Type `Answering Service`; required tags and sorting fields. After creation, send an internal case-created email. Store the Zendesk id internally, but remove it from the response returned to Retell. Tell the caller the team will be in touch as soon as possible without exposing case terminology or reference numbers. |
